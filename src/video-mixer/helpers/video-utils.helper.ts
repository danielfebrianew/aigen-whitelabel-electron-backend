...
const ffmpegPath = process.env.FFMPEG_PATH || 'D:\\ffmpeg\\bin\\ffmpeg.exe';
const ffprobePath = process.env.FFPROBE_PATH || 'D:\\ffmpeg\\bin\\ffprobe.exe';
ffmpeg.setFfmpegPath(ffmpegPath);
ffmpeg.setFfprobePath(ffprobePath);

const _initLogger = new Logger('VideoUtilsHelper[Init]');
_initLogger.log(`[FFmpeg] Path  : ${ffmpegPath}`);
_initLogger.log(`[FFmpeg] Probe : ${ffprobePath}`);
_initLogger.log(`[FFmpeg] Exists ffmpeg  : ${fs.existsSync(ffmpegPath)}`);
_initLogger.log(`[FFmpeg] Exists ffprobe : ${fs.existsSync(ffprobePath)}`);
...
export class VideoUtilsHelper {
  ...
  mergeVideoFiles(inputs: string[], output: string): Promise<void> {
    return new Promise((resolve, reject) => {
      this.logger.log(`[mergeVideoFiles] Merging ${inputs.length} clips -> ${output}`);
      inputs.forEach((p, i) => {
        this.logger.log(`  Clip[${i}] exists=${fs.existsSync(p)} path=${p}`);
      });
      ...
      command
        ...
        .on('end', () => {
          this.logger.log(`[mergeVideoFiles] Done -> ${output}`);
          resolve();
        })
        .on('error', (err) => {
          this.logger.error(`[mergeVideoFiles] Error: ${err.message}`);
          reject(err);
        });
    });
  }
